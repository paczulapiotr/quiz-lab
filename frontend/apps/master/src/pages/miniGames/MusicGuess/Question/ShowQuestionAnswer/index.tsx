...
const ShowQuestionAnswer = ({ gameId }: Props) => {
  ...
  const playersData = useMemo(() => data?.state?.rounds.find((round) => round.roundId === data?.state?.currentRoundId)?.answers.map(x => ({
    id: x.playerId,
    name: players.find(p => p.id === x.playerId)?.name ?? "",
    answerId: x.answerId ?? undefined,
    answerPoints: x.points,
    roundPoints: data.state?.rounds.reduce((acc, round) => acc + (round.answers.find(a => a.playerId === x.playerId)?.points ?? 0), 0) ?? 0,
  })) ?? [], [data?.state?.currentRoundId, data?.state?.rounds, players]);

  const answers = useMemo(() => data?.definition?.rounds
    ...
    [data?.definition?.rounds, data?.state?.currentCategoryId, data?.state?.currentQuestionId, data?.state?.currentRoundId]);
  
  return (
    <Component answers={answers ?? []} players={playersData} />
  );
}
