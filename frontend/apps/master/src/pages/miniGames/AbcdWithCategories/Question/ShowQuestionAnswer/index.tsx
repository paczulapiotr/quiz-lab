...
const ShowQuestionAnswer = ({ gameId }: Props) => {
  ...
  const playersData = players.map(player => ({
    ...
    roundPoints: data?.state?.rounds.reduce((acc, round) => acc + (round.answers.find(a => a.playerId === player.id)?.points ?? 0), 0) ?? 0,
  }));

  return (
    <Component answers={answerDefs} players={playersData} />
  );
};
